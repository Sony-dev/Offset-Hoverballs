...
TOOL.ClientConVar = {
  ...
  ["adjust_speed"] = "0.8",
  ["brake_resistance"] = "15",
  ["offset_hoverball_nocollide"] = "true",
  -- Toggle numpad keys
  ["key_toggle"] = "0",
  ["key_heightdown"] = "0",
  ["key_heightup"] = "0",
...
function TOOL:LeftClick(trace)
  ...
  if (SERVER) then
    ...
    if (IsValid(trace.Entity) and trace.Entity:GetClass() == "offset_hoverball") then
      ...
      trace.Entity:SetOverlayText(string.format(
                                    "Hover height: %g\nForce: %g\nAir resistance: %g\nAngular damping: %g\n Brake resistance: %g",
                                    trace.Entity.hoverdistance, trace.Entity.hoverforce, trace.Entity.damping,
                                    trace.Entity.rotdamping, trace.Entity.brakeresistance))
      ...
      trace.Entity.key_heightup = numpad.OnDown(ply, self:GetClientNumber("key_heightup"), "offset_hoverball_heightup",
                                                trace.Entity, true)
      trace.Entity.key_heightbackup = numpad.OnUp(ply, self:GetClientNumber("key_heightup"),
                                                  "offset_hoverball_heightup", trace.Entity, false)

      trace.Entity.key_heightdown = numpad.OnDown(ply, self:GetClientNumber("key_heightdown"),
                                                  "offset_hoverball_heightdown", trace.Entity, true)
      trace.Entity.key_heightbackdown = numpad.OnUp(ply, self:GetClientNumber("key_heightdown"),
                                                    "offset_hoverball_heightdown", trace.Entity, false)

      trace.Entity.key_brake = numpad.OnDown(ply, self:GetClientNumber("key_brake"), "offset_hoverball_brake",
                                             trace.Entity, true)
      trace.Entity.key_brakerelase = numpad.OnUp(ply, self:GetClientNumber("key_brake"), "offset_hoverball_brake",
                                                 trace.Entity, false)

      if (key_toggle) then
        trace.Entity.key_toggle = numpad.OnDown(ply, self:GetClientNumber("key_toggle"), "offset_hoverball_toggle",
                                                trace.Entity)
      end
      ...
      local ball = Create_offset_hoverball(self:GetOwner(), trace.HitPos, self:GetClientNumber("height"),
...
function TOOL:RightClick(trace) end

function TOOL.BuildCPanel(panel)

  panel:AddControl("Header", {
    Description = "Hoverballs spawned by this tool will try to maintain their height relative to the terrain."
  })

  panel:AddControl("ComboBox", {
    MenuButton = 1,
    Folder = "hoverball_offset",
  ...
  panel:AddControl("PropSelect", {
    Label = "Model",
    ConVar = "offset_hoverball_model",
    Models = list.Get("DistanceHoverballModels"),
    Height = 5
  })

  panel:AddControl("Slider",
                   {Label = "Force", Type = "Float", Min = "5", Max = "1000", Command = "offset_hoverball_force"})

  panel:AddControl("Slider",
                   {Label = "Height", Type = "Float", Min = "5", Max = "1500", Command = "offset_hoverball_height"})
  ...
  panel:AddControl("checkbox", {Label = "Hovers over water", Command = "offset_hoverball_detects_water"})

  panel:AddControl("checkbox", {Label = "Spawn nocollided", Command = "offset_hoverball_nocollide"})

  panel:AddControl("Numpad", {
    Label = "Increase hover height",
    Command = "offset_hoverball_key_heightup",
    Label2 = "Decrease hover height",
    Command2 = "offset_hoverball_key_heightdown"
  })
  panel:AddControl("Numpad", {
    Label = "Toggle hoverball on/off",
    Command = "offset_hoverball_key_toggle",
    Label2 = "Brake (Hold)",
    Command2 = "offset_hoverball_key_brake"
  })
...
local function IsValidHoverballModel(model)
  for mdl, _ in pairs(list.Get("DistanceHoverballModels")) do if (mdl:lower() == model:lower()) then return true end end
  return false
end
...
if (SERVER) then
  ...
  function Create_offset_hoverball(ply, pos, hoverdistance, hoverforce, damping, rotdamping, detectswater, adjustspeed,
    ...
    ball:SetOverlayText(string.format(
                          "Hover height: %g\nForce: %g\nAir resistance: %g\nAngular damping: %g\nBrake resistance: %g",
                          ball.hoverdistance, ball.hoverforce, ball.damping, ball.rotdamping, ball.brakeresistance))

    if (IsValid(ply)) then ball:SetPlayer(ply) end
  ...
  duplicator.RegisterEntityClass("offset_hoverball", Create_offset_hoverball, "pos", "hoverdistance", "hoverforce",
